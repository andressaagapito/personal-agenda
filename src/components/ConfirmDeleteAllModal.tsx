...
export function ConfirmDeleteAllModal({ isOpen, onClose, onConfirm }: ConfirmDeleteAllModalProps) {
  const { t } = useLanguage();
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 overflow-y-auto" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0 }}>
      ...
      >
        <div className="flex items-center justify-between mb-6 flex-shrink-0">
          <h2 className={`text-xl sm:text-2xl ${theme === 'dark' ? 'text-gray-100' : 'text-gray-800'}`}>{String(t('deleteAllAgendas'))}</h2>
          <button
            onClick={onClose}
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <p className={`${theme === 'dark' ? 'text-gray-200' : 'text-gray-700'} mb-2`}>
                {String(t('confirmDeleteAllAgendas'))}
              </p>
              <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                {String(t('deleteAllAgendasWarning'))}
              </p>
            </div>
          </div>
